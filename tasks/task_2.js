...
function task(host,dao,activeTask){
	...
	function stackInput(Input){
		var items = Input.lifo.getItems().sort(function(a,b) {
							if(a.point.localeCompare(b.point) > 0)
								return -1;
							else
								return 1;
						});
		...
		for(var i=0; i <  items.length; i++){
			if(point.localeCompare(items[i].point) != 0){
				pos++;
				points[pos] = new Array();
				point = items[i].point;
			}
			points[pos].push(items[i]);
		}
		Input.lifo.setItems(points);
	}
	...
	function show(res,Dataset,Fingerprint,Input) {
		...
		dao.loadContributions(activeTask,Fingerprint,function(contributions){
			if(Input.lifo.isEmpty()){
				...
				Input.lifo.setItems(Input.lifo.getItems().reverse());
			}
...
*/
			var items = Input.lifo.pop();
			Input.fifo.push(items);
			...
			values.video_id = Dataset[item.video]._id;
			...
			values.point = item.point;
			values.instant = item.instant;

			itemsJson = JSON.stringify(items);

			values.items = itemsJson;
